...
LiteWAF 是运行在 nginx 容器内的 OpenResty Lua 模块，在反向代理层对请求做 CC 限速与攻击特征检查，并维护一个公开的统计页。它工作在 nginx access 阶段，所有状态保存在 `lua_shared_dict` 中，不写盘，进程重启后清零。应用层安全（参数化查询、输出转义、日志脱敏）由 Hyperf 侧负责，不在本模块范围内。
...
LiteWAF/
├── lua/
│   ├── litewaf.lua      # 核心模块：配置、规则、检查逻辑、统计输出
│   ├── waf.lua          # access_by_lua_file 入口，调用 litewaf.access()
│   └── stats.lua        # content_by_lua_file 入口，调用 litewaf.stats()
...
`docker/compose.yaml` 中 nginx 服务使用 `openresty/openresty:1.27.1.2-alpine`，相关挂载：
...
`docker/nginx/default.conf` 的 80 与 443 两个 server 都需要 `access_by_lua_file`，443 另有三个统计 location：
...
独立部署时需在 http 级提供 `lua_package_path`、`lua_shared_dict litewaf 16m` 与 `init_by_lua_block`（见 `nginx/nginx.conf`），再挂接上述 location。

改动 Lua 文件后需要 reload 才生效：

```bash
docker compose -f docker/compose.yaml exec nginx nginx -s reload
```

OpenResty 默认开启 `lua_code_cache`，`git pull` 只更新磁盘文件，不 reload 则 worker 仍运行旧代码。reload 保留计数，重启清零。

升级 OpenResty 镜像 tag 时（当前固定 `1.27.1.2-alpine`，上游存在 `-7` 补丁系列），核对 `nginx/nginx.conf` 与镜像内置配置的差异（lua 指令、临时路径、默认 include），必要时重新挂载。

## 配置

全部配置集中在 `lua/litewaf.lua` 顶部的 `CONFIG` 表：
...
规则定义在 `lua/litewaf.lua` 的 `_M.RULES`，位于 `-- RULES-BEGIN` / `-- RULES-END` 标记之间，每条为 `{ "类目", [[PCRE]] }`，类目为小写字母：`sqli`、`xss`、`traversal`、`rce`、`probe`。正则使用 ngx.re 语法，编译 flags 为 `ji`（忽略大小写 + JIT）。

编写时注意：规则必须保持单行双元素表，`tests/litewaf_regex_test.php` 按 `RULES-BEGIN/END` 标记与该格式解析；规则内容含 `]]` 时（如字符类 `[a-z]` 结尾），Lua 长括号改用 `[==[ ]==]`；规则按顺序匹配、命中即停，新增规则的位置影响计数归属；修改或新增规则时，同步在 `tests/litewaf_regex_test.php` 补充正样本（应拦截）与反样本（不应拦截）。

回归测试与语法检查：

```bash
php LiteWAF/tests/litewaf_regex_test.php    # 规则正负样本（PHP PCRE，与 ngx.re 同源）
lua5.1 LiteWAF/tests/litewaf_logic_test.lua # 检查流程/CC/体检查/日志/统计逻辑（stub ngx）
