...
最新版本 Fabric Loader 的 Javadoc 可以在 Develop 网站上找到。

可以通过 `FabricLoader.getInstance()` 获取当前的 Fabric Loader 实例。 例如，可以使用 `FabricLoader.getInstance().isModLoaded` 来检查另一个模组是否正在运行。

## 模组

模组是一个在根目录下包含 `fabric.mod.json` 元数据文件的 JAR 包，该文件声明了模组应如何加载。 该文件包含模组 ID 和版本号，以及入口点和 Mixin 配置。
...
Fabric Loader 采用基于入口点的系统，用于向 Fabric Loader 或其他模组暴露特定代码以进行初始化。
...
Fabric Loader 使用了原始 Sponge Mixin 的修改分支。 不过，上游 Mixin Wiki 的大部分内容仍然适用。
...
当在混淆处理游戏的非开发环境中启动时，Fabric Loader 会将游戏 JAR 重新映射为中介名。 针对混淆处理游戏设计的模组预期被映射到中介，这将与该环境兼容。 重新映射后的 JAR 会被缓存并保存在 `${gameDir}/.fabric/remappedJars/${minecraftVersion}` 中，以便在多次启动间重复使用。
