...
以上示例是该任务最基本的用法，将生成一个 `fabric.mod.json` 文件，其中包含指定的模组 ID 和版本。 这个 `json` 代码块支持所有在 Fabric Mod JSON 格式中定义的字段。
...
`net.fabricmc.loom.task.tool.ModEnigmaTask` 是一个高级任务，可用于针对映射文件启动 Enigma。 这可以用来生成模组提供的 javadoc。
