...
There are also guides for specific configurations, such as Docker/Pterodactyl, or on specific VPS/KVM providers, such as OVH or Oracle Cloud. 
	...
	If you are using a Minecraft server hosting provider (e.g. Aternos, or Apex Hosting), you should refer to the hosting provider setup on the setup page instead.
...
1. Search for "Windows Defender Firewall with Advanced Security" in the start menu, and open it. ([Image]())
2. Click on "Inbound Rules" in the left sidebar. ([Image]())
3. Click on "New Rule..." in the right sidebar. ([Image]())
4. Select "Port" as the rule type and click "Next". ([Example]())
5. Select "UDP" and "Specific local ports", and enter the port you want to open (in our example, 19132). Click "Next". ([Example]())
6. Select "Allow the connection" and click "Next". ([Example]())
7. Select the profiles you want to apply the rule to (e.g. "Domain", "Private", "Public"), and click "Next". ([Example]())
8. Enter a name for the rule (e.g. "Geyser"), and click "Finish". ([Image]())
...
To allow incoming connections, click `Allow` when getting [these prompts]().
...
[image: Pterodactyl panel, Network section]
